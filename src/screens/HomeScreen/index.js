import React, {useState} from 'react';

import {Screen, CardContainer} from './styles';

import Header from '../../components/Header';
import UserCard from '../../components/UserCard';

export default function HomeScreen({navigation}) {
  const [doorStatus, setDoorStatus] = useState(false);

  const onChangeDoorStatus = () => {
    setDoorStatus(!doorStatus);
  };

  return (
    <Screen>
      <Header
        registerButtonText="+ Add New User"
        doorOpenText="Open "
        doorCloseText="Close "
        doorStatus={doorStatus}
        onPressRegister={() => navigation.navigate('Register')}
        onPressDoor={onChangeDoorStatus}
      />
      <CardContainer>
        <UserCard
          isValid={false}
          name="Isuru Udara"
          time="09.30 AM"
          userImageUrl={{
            uri: 'data:image/jpeg;base64,',
          }}
        />
        <UserCard
          isValid={true}
          name="Gomedha Sasmini"
          time="10.30 AM"
          userImageUrl={{
            uri: 'https://www.bentbusinessmarketing.com/wp-content/uploads/2013/02/35844588650_3ebd4096b1_b-1024x683.jpg',
          }}
        />
        <UserCard
          isValid={true}
          name="Janaka Pethum"
          time="02.30 PM"
          userImageUrl={{
            uri: 'https://www.incimages.com/uploaded_files/image/1920x1080/getty_624206636_200013332000928034_376810.jpg',
          }}
        />
        <UserCard
          isValid={false}
          name="Gayan Dhanushka"
          time="01.30 PM"
          userImageUrl={{
            uri: 'https://www.bobbin.lk/wp-content/uploads/2019/02/person1.jpg',
          }}
        />
        <UserCard
          isValid={true}
          name="Dasun Abesinghe"
          time="08.30 PM"
          userImageUrl={{
            uri: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR-O6RxdxvT6zRrN92202IXN1RUA7TSHteOZw&usqp=CAU',
          }}
        />
        <UserCard
          isValid={true}
          name="Hasitha Supun"
          time="08.30 PM"
          userImageUrl={{
            uri: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRuWlKVGJeILLo6n3_-9GgvdvWyz7MJbC1o7g&usqp=CAU',
          }}
        />
        <UserCard
          isValid={false}
          name="Mithila Sachiira"
          time="08.30 PM"
          userImageUrl={{
            uri: 'https://miro.medium.com/max/1024/1*gDp6Eg2mkO6b3UnxWPO8ng.jpeg',
          }}
        />
      </CardContainer>
    </Screen>
  );
}
